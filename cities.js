...
function convertData(data) {
    ...
    var tempCountries = [];
    var tempStates = [];
    data.forEach(function (currentCountry) {
        if (!tempCountries.includes(currentCountry.Country)) {
            tempCountries.push(currentCountry.Country);
            tempStates.push(currentCountry.State);
            countries.push({
                "Name": currentCountry.Country,
                "States": [{
                    "Name": currentCountry.State,
                    "Cities": [{
        ...
        } else if (!tempStates.includes(currentCountry.State)) {
            tempStates.push(currentCountry.State);
            var countryIndex = countries.findIndex(function (country) {
                return country.Name == currentCountry.Country;
            });
            ...
            var stateIndex = countries[countryIndex].States.findIndex(function (State) {
                return State.Name == currentCountry.State;
            })
            countries[countryIndex].States[stateIndex].Cities.push({
                "Name": currentCountry.Name,
                "Population": currentCountry.Population
            });
        }
...
function sortData(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        data[i].States.sort(function (a, b) {
            if (a.Name < b.Name)
                return -1;
            if (a.Name > b.Name)
                return 1;
    ...
    for (var i = 0; i < data.length; i++) {
        for (var j = 0; j < data[0].States.length; j++) {
            data[i].States[j].Cities.sort(function (a, b) {
